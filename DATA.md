...
상승·하강량은 경사 확인 구간의 평활화된 노드 고도차를 누적합니다. 짧은 구간에도 최소 60m를 적용한 경사값을 실제 구간 길이에 곱하면 상승량이 축소되므로, 고도값이 있는 구간은 경사에서 역산하지 않습니다. 미확인 구간은 누적에서 제외합니다.

카페·디저트는 카드 원자료의 독립 소비분류 근거가 없습니다. 실제 성수권역 카카오 POI가 확인되는 경우 `식사·카페 러닝` 후보로 포함하며, 식당·카페 모두 소비통계에 따른 가산점 없이 장소·경로 적합도로 평가합니다. 집계 소비통계는 미션 메뉴 구성의 근거로만 사용합니다.
